...
- `stages`：两个 stage，`dev-isolated`（`environment.required: false`，runner 仅跑 `modules.<m>.test.unit`）和 `dev-mock-integration`（`environment.required: true`，runner 走 `modules.<m>.test.integration`）。
