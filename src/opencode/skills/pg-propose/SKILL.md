...
- 生成后必须在 `tasks.md` 的 `scenario-execute` 章节步骤组 1 中确认：
  `- [ ] <N>.1 确认 .pg/changes/<change>/scenario.yaml 存在且每个 Scenario 含 6 段`

4. 写盘命令：
```bash
cat > .pg/changes/CHANGE_NAME/scenario.yaml << 'EOF'
...
5. 校验文件存在：
```bash
ls -la .pg/changes/CHANGE_NAME/scenario.yaml
```
...
每个 change 在 `.pg/changes/<change>/` 下**至少生成**以下 5 个产物文件（前 4 个 + 条件性的 scenario.yaml）：
...
| `scenario.yaml` | `.pg/changes/<change>/scenario.yaml` | 阶段 2f（仅当 `execution-manifest.yaml` 含 `type: scenario` 的 track） | ⚠️ 条件必填 |
...
**生成路径**：阶段 2d 在生成 `tasks.md` 后，**同时**写盘 `.pg/changes/<change>/scenario.yaml`。
...
- `cat > .pg/changes/<change>/scenario.yaml` 写盘
