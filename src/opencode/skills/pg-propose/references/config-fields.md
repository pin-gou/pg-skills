...
| `build_rules` | list | build 阶段 prompt 注入 | 按 `target_agent` + `position` 注入 sub-agent |
...
| pg-propose 阶段 | 读取的字段 |
|---|---|
| 1e 获取管线配置 | `modules`, `tracks`, `stages`, `environments`, `rules`, `proposal_rules`, `build_rules`, `test_strategy`, `coding_standards` |
...
| 2 tasks 生成 | `rules.tasks`, `stages[*]`, `tracks[*]`, `tracks[*].on_conditions`, `modules[*]`, `environments[*]` |
