...
for stage in enabled_stages:  # 改用 enabled_stages 而非 config.stages
    for track_id in stage.tracks:
        ...
        # simple track:   走 1 个 simple 章节模板 (runner 直接执行 commands)
...
- canonical form 一步到位生成（heading 含 `(simple track: runner 直接执行 commands)` + body 单 `- 无` 行），与 runner 的 `_noopify_simple_track_sections` canonical form 完全对齐
...
```markdown
## {N}. {stage.name}.{track_id} - {stage.name} {track_id}  (simple track: runner 直接执行 commands)
```
...
```markdown
## 9. dev.openapi-gen - dev openapi-gen  (simple track: runner 直接执行 commands)
...
- runner 直接执行 `tracks.{track_id}.commands`，按命令的 `on_failure` 处理结果（`fail` / `continue` / `retry`）
