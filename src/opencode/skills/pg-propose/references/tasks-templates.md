...
v3.x 起，仅当 `tracks.<id>.code_review_enabled=true` 时此章节才出现在 tasks.md。runner 不展开命令，review agent 自己读 `.pg/code-review/code-review.yaml` 解析 profile。
...
**何时本章节不出现（v3.4 新增）**：
- `tracks.<id>.verify_enabled=false` → 脚本跳过此 sub，章节号 N 减少 1
- 不关闭 verify 时本章节默认存在（与 v3.x 一致）
- simple track → 不走 5 sub 模板，本章节天然不出现
...
**何时本章节不出现（v3.4 新增）**：
- `tracks.<id>.gate_enabled=false` → 脚本跳过此 sub，章节号 N 减少 1
- simple track → 不走 5 sub 模板，本章节天然不出现

**质量门约束**：
- `verify_enabled` + `gate_enabled` 同时为 false **不允许**（validator 拒绝）。review 单独存在不算质量门——静态审查不替代运行时验证。
- 至少保留 verify **或** gate 一项作为运行时质量门。
...
## 25. real-integration.scenario-test:scenario-prepare - 真机场景准备
...
## 26. real-integration.scenario-test:scenario-execute - 真机场景执行

#### 步骤组 1：scenario.md 读取

- [ ] 26.1 确认 `.pg/changes/<change>/scenario.md` 存在且每个 Scenario 含 6 段（scenario_id / critical / given / when / then / evidence；and 可选）
...
- [ ] 26.5 产出结构化 JSON 证据到 `2-build/<scenario_id>-evidence.json`
...
- [ ] 26.9 scenario-execute agent 写盘报告到 `2-build/<seq>-real-integration.scenario-test-scenario-execute.md`

## 27. real-integration.scenario-test:scenario-fix - 真机场景修复（仅 execute escalate 时存在）

#### 步骤组 1：诊断

- [ ] 27.1 读源 scenario-execute 报告 + design.md + proposal.md
- [ ] 27.2 定位失败根因（业务逻辑 / API 契约 / 前后端契约 / DB / 配置）

#### 步骤组 2：修复

- [ ] 27.3 在 track.modules 路径下改代码
- [ ] 27.4 跑单元测试 + lint（必须通过）
- [ ] 27.5 写修复报告到 `2-build/<seq>-real-integration.scenario-test-scenario-fix-<n>.md`

#### 步骤组 3：循环

- [ ] 27.6 record(scenario-fix, "completed" / "failed") → 编排器自动 dispatch scenario-execute 重跑
- [ ] 27.7 max_fix_retries 耗尽 → workflow_failed（不进入 gate，scenario 失败**不**接受 gap）
