...
| **修复记录（gate 触发）** | `{track.id}-{N}-gate-fix.md` | fix-gate agent | gate FAIL | "我**修复了** gate 列出的 G-N gap"；description：fix-gate agent 使用独立的 `_PROMPT_BLOCK_FIX_GATE` 模板，issue 字段命名走 gate 视角（`gate_gap_id` / `audit_step` / `file_pos` / `fix_hint`），与 verify-fix 的 `_PROMPT_BLOCK_FIX` 严格区分 |
...
| `--skill` | ❌ | 调用方 skill 名称（如 `pg-build`）；注入为 `PG_SKILL_NAME` 环境变量 |
...
| 触发时机 | 动作 |
|---------|------|
| dev/verify/fix 需要启动某 role 服务 | `pg-invoke-hook.py invoke-hook --change X --env Y --role backend --instance backend-1 --action start --skill pg-build` |
| 看日志 | `pg-invoke-hook.py invoke-hook --change X --env Y --role backend --instance backend-1 --action logs --tail-lines 100 --skill pg-build` |
...
```json
{
  ...
  "invocation": {
    "command_template": "python3 .pg/skills/src/runtime/bin/pg-invoke-hook.py invoke-hook --change <CHANGE> --env <ENV> --role <ROLE> --instance <INSTANCE> --action <ACTION> [--stage <STAGE>] [--tail-lines <N>] [--skill <SKILL>]",
