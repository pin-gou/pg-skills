...
while true; do
  ...
  case $ACTION in
    ...
    "env_hook_failed")  → 输出失败报告（env-hook 启动期失败，流程终止）→ break
...
> **重大变更（v2 path）**：env-hook 不再返回独立 `phase_result` action。
> - 启动期 `prepare_env` 在 `pg_build_bootstrap` 内部同步执行，LLM 无感（仅首次 dispatch action 中携带 `environment` 摘要字段）。
> - env-hook 失败统一返回 `env_hook_failed` action（携带 log_path / exit_code / reason）。
> - `phase_result` 路径**仅 v1 保留**（v1 cmd_next/cmd_record 流程未变）。
...
| 命令 | 用法 | 说明 |
|------|------|------|
| `next` | `$RUNNER next <change>` | **主循环入口**。返回 dispatch action 或终态。v2 path 首次调用会自动完成初始化（feature branch 创建、state 初始化、init commit、`prepare_env` 同步执行 + 记账到 context-chain）；v1 path 首次调用会返回 `phase_result`（见「执行流程」）。env-hook 失败统一返回 `env_hook_failed` action |
...
> **v2 path**：env-hook phase（prepare_env / clean_env）不再通过 record 推进——它们在 `pg_build_bootstrap` 内部同步执行。LLM 不应发送任何 `record` 调用给 env-hook item。
...
runner 在 `cmd_record` 和 `cmd_next` 入口都会校验 **state vs tasks.md 一致性**（env-hook 不参与此校验：env-hook 不写 tasks.md，也不接受 record 调用）。如果检测到漂移（如 state 说 sub=verify 但 tasks.md §4 gate 才是第一个未完成 section），返回 `action: error` + `drift_kind`：
...
## environment 字段挂载规范（v2 path）

env-hook 在 `pg_build_bootstrap` 内部执行完成后，runner 把环境摘要挂载到 `cmd_next_v2` 的**首次** `dispatch` action 中。LLM 仅在需要查看环境信息时读取该字段，无需发起额外请求。
...
```json
{
  "environment": {
    "name": "dev-local",
    "prepare_env_log_path": ".pg/changes/<change>/2-build/dev-prepare_env-1.log",
    "instances": {
...
- 仅在 v2 path 的 prepare_env 成功后的**首次** dispatch action 中挂载完整 environment 摘要。
- 后续 dispatch 不再挂（LLM 通过 `state.context.environment_summary` 自行查阅——虽然 v2 路径下通常不需要）。
- `cmd_record_v2` 不挂 environment。
- 启动期 prepare_env 失败时，runner 不挂 environment，转而返回 `env_hook_failed` action（携带 log_path / exit_code）。

**与 v1 phase_result.environment 的对应关系**：

| v1 path | v2 path |
|---|---|
| `phase_result{terminate=false, environment={...}}` 由 LLM 缓存到本地 | `dispatch.environment={...}` 直接随 action 返回 |
| LLM 需手动调 `record completed` 推进 | LLM 直接派送 sub-agent |
...
- `prepare_env`/`clean_env` 在 v2 path 下由 `pg_build_bootstrap` 内部同步执行（不再返回 `phase_result` action）。启动期 `prepare_env` 失败时返回 `env_hook_failed` action（fatal=true），LLM 流程终止；final-gate 之后的 `clean_env` 失败仅记 warning，不阻塞 done 返回。v1 路径仍走 `phase_result` + `record completed` 模式。
