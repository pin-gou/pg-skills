...
| `dev` | design.md / tasks.md dev 章节 | 修改源码 + 更新 tasks.md 复选框 | 调用 `pg-invoke-hook.py invoke-hook` 触发 start/stop/logs/tail（runner 已注入 CLI 示例到 prompt_template）；改完跑 `stage.test_commands` 验证 |
...
| 触发时机 | 动作 |
|---------|------|
| dev/verify/fix 需要启动某 role 服务 | `pg-invoke-hook.py invoke-hook --change X --env Y --role backend --instance backend-1 --action start` |
| 看日志 | `pg-invoke-hook.py invoke-hook --change X --env Y --role backend --instance backend-1 --action logs --tail-lines 100` |
| 整个 change 跑完（可选） | 依次 `pg-invoke-hook.py invoke-hook ... --action stop` 收尾 |
...
```json
{
  ...
  "invocation": {
    "command_template": "python3 .pg/skills/src/runtime/bin/pg-invoke-hook.py invoke-hook --change <CHANGE> --env <ENV> --role <ROLE> --instance <INSTANCE> --action <ACTION> [--stage <STAGE>] [--tail-lines <N>]",
