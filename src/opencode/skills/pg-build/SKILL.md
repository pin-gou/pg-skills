...
| **修复记录（gate 触发）** | `{track.id}-{N}-gate-fix.md` | fix-gate agent | gate FAIL | "我**修复了** gate 列出的 G-N gap" |
...
### 步骤 1：runner 已渲染完整 prompt

`next` 返回的 `prompt_template` 字段包含了 sub-agent 所需的全部内容：Track 配置 / Module 配置 / Stage 配置 / 实例拓扑 / Hooks 调用约定（含 timeout_seconds）/ 任务清单 / 模块路径约束 / 返回格式。orchestrator **不需要**手工拼接。
...
| 场景 | 动作 |
|------|------|
| dispatch / dispatch_fix / dispatch_final_gate | `prompt = action["prompt_template"]` → 叠加上 `prompt_injection.prepend/append`（如果有）→ 用 Task tool 派送。**不要修改 prompt 内容**。完成后 `$RUNNER record ...` |
...
> 以下语法仅供开发调试参考。orchestrator 不需要手动使用；runner 已在 `prompt_template` 中渲染好。
...
| `dev` | design.md / tasks.md dev 章节 | 修改源码 + 更新 tasks.md 复选框 | 调用 `runner invoke-hook` 触发 start/stop/logs/tail（runner 已注入 CLI 示例到 prompt_template）；改完跑 `stage.test_commands` 验证 |
| `verify` | tasks.md verify 章节 | `2-build/{track.id}-(N+1)-verify.md` | 按需 `runner invoke-hook` 启停服务；按 tasks.md V-* 清单逐项 curl/检查 |
| `gate` | `2-build/{track.id}-{N}-verify.md` | `2-build/{track.id}-(N+1)-gate-assessment.md` | **只读不写**源码；`cat >` 自行写盘 |
| `fix-gate` | `2-build/{track.id}-{N}-gate-assessment.md`（G-N 章节） | `2-build/{track.id}-(N+1)-gate-fix.md` | 仅用 `runner invoke-hook --action start\|stop` |
...
新 schema 中**没有** `track.rebuild_and_restart` 字段——deployment 启停的执行由 **LLM 主导**。LLM 通过 `runner invoke-hook` CLI 触发 role action，runner 内部负责从 project.yaml 反查 spec 并调用 pg-run-hook.py。
...
| 触发时机 | 动作 |
|---------|------|
| dev/verify/fix 需要启动某 role 服务 | `runner invoke-hook --change X --env Y --role backend --instance backend-1 --action start` |
| 看日志 | `runner invoke-hook --change X --env Y --role backend --instance backend-1 --action logs --tail-lines 100` |
| 整个 change 跑完（可选） | 依次 `runner invoke-hook ... --action stop` 收尾 |
...
```json
{
  ...
  "invocation": {
    "command_template": "python3 .opencode/skills/pg-build/scripts/pg-pipeline-runner.py invoke-hook --change <CHANGE> --env <ENV> --role <ROLE> --instance <INSTANCE> --action <ACTION> [--stage <STAGE>] [--tail-lines <N>]",
...
fix agent **完全继承 base dispatch 模板（步骤 2）的所有字段**：变更名称 / Track 配置 / Module 配置 / Stage 配置 / Hooks 调用约定 + 实例拓扑 / 产物路径 / 模块路径约束 / 验证要求。在此基础上，**追加** FIX ISSUE REQUEST 块。
...
> 6. **模块路径约束**（硬规则）
> 7. **必读报告**：`2-build/{track.id}-{N}-verify.md`（上一轮 verify 报告，含 ESCALATE Issue 详情）
> 8. **写盘要求**：修复记录写到 `2-build/{track.id}-(N+1)-verify-fix.md`（用 `cat >` 自行写盘）
> 9. **FIX ISSUE REQUEST 块**（下方模板）

**FIX ISSUE REQUEST 块**（runner 已将 issue 详情注入 context）：
...
- change_name: {context.change-name}
