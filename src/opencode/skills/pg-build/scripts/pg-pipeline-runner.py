...
def _usage(msg: str = "") -> None:
    ...
    print("  python3 pg-pipeline-runner.py env-action <change> --phase prepare_env|clean_env|restart --stage <stage> --env <env> [--timeout <秒>]", file=sys.stderr)
    print("  python3 pg-pipeline-runner.py env-action-result <change> --phase prepare_env|clean_env|restart --stage <stage> --env <env> --success true|false [--log-path <path>] [--exit-code <code>] [--started-ts <ts>] [--error <msg>]", file=sys.stderr)
    ...
    print("env-action --phase: prepare_env | clean_env | restart", file=sys.stderr)
