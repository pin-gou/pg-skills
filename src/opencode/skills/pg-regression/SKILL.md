...
```
.pg/regression/<suite>.json          # 问题清单 (Phase 3.2 写入, runner 修复后移除已修 issue)
.pg/regression/summary-<datetime>.md # 人类可读汇总报告 (Phase 3.3)
.pg/regression/results/<datetime>-<suite>-<id>-pr<N>.json  # runner 单 issue 修复结果
```
...
```bash
mkdir -p temp && rm -f temp/{suite}-test-output.log temp/{suite}-phase1-failures.json temp/{suite}-phase1-known-issues.json temp/{suite}-fix-results.json
```
...
python3 .pg/skills/src/opencode/scripts/pg-parse-test-results.py parse \
  --type <推导出的 type> \
  --log-file temp/{suite}-{test_key}-test-output.log \
  --out temp/{suite}-{test_key}-phase1-failures.json
```

所有 failures 合并去重，输出到 `temp/{suite}-phase1-failures.json`：
...
json.dump(out, open('temp/{suite}-phase1-known-issues.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=2)
" \
  || echo '{"skipped_targets": []}' > temp/{suite}-phase1-known-issues.json
```
...
编排器阅读每个 agent 的报告，提取结构化字段，构造 `temp/{suite}-fix-results.json`：
...
suite = "{suite}"
fix_results = json.load(open(f"temp/{suite}-fix-results.json", encoding="utf-8"))
known_issues = json.load(open(f"temp/{suite}-phase1-known-issues.json", encoding="utf-8"))
...
python3 .opencode/skills/pg-regression/scripts/pg-regression-summary.py \
  ...
  --out ".pg/regression/summary-$DATE_TAG.md"
```
...
📋 汇总报告: .pg/regression/summary-<datetime>.md
```
...
" .pg/regression/*.json 2>/dev/null; then
  nohup python3 .opencode/skills/pg-regression/scripts/pg-fix-regression-runner.py \
    > .pg/regression/last-runner.log 2>&1 &
  echo "✅ pg-fix-regression-runner.py 已启动 (PID=$!)"
  echo "   日志: .pg/regression/last-runner.log"
  echo "   结果: .pg/regression/results/"
else
...
每轮 Phase 1 产出独立的 `temp/{suite}-{test_key}-phase1-failures.json`，Phase 2 调度 fix-test 时 `testOutputFile` 也指向对应 test_key 的输出。
