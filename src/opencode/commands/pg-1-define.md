...
```
    ...
    └─────────────────┼────┼────┼──────────────────────┘
           ...
           ▼               ▼                    ▼
    ┌───────────────────┐  ┌──────────────┐  ┌────────────┐
    │ /2b-pg-quick-build│  │ /2-pg-propose│  │ 仅理清思路 │
    │ 快速实现          │  │ 完整提案+实现│  │ 无需下一步 │
    └───────────────────┘  └──────────────┘  └────────────┘
```
...
**口头 summary 完成后，告知用户下一步可加载 pg-propose skill 生成正式产物文件**（proposal.md + design.md + tasks.md），或加载 pg-quick-build skill 直接实施，或者只是理清思路，不需要产出。
...
- → **不加载任何 skill 直接实施**：这次划定的范围不适合使用 pg-* skills 推进，将详细计划输出到 temp 目录
- → **仅理清思路**：用户只需要思考，不需要产出
