...
按 dispatch_file 里的 tasks.md 验证项逐项执行。**环境不具备时**（如 backend 未启动、agent gRPC 不可达、wscat 缺失）：

- **不要**把环境受限当作代码 bug 直接 `status: "escalate"`
...
- 继续执行剩余验证项收齐全部失败
- 在报告里列出每个 Issue（verification_step / expected / actual / affected_tasks）
