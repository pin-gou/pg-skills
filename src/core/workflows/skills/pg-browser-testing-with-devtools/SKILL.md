...
Chrome DevTools MCP 需要在项目 `{{pg:tool.config_file}}` 中注册（已配好），使用 Google 官方发布的公开包 `chrome-devtools-mcp`：
...
```json
{
  "mcp": {
    "chrome-devtools": {
      "type": "local",
      "command": ["npx", "-y", "chrome-devtools-mcp@latest", "--headless", "--executablePath", "{{pg:tool.chrome_path_reference}}", "--chromeArg", "--no-sandbox"]
    }
  }
}
```
...
| `--executablePath {{pg:tool.chrome_path_reference}}` | 通过 {{pg:tool.name}} 的环境变量引用语法使用 `CHROME_PATH`，不硬编码路径 |
