...
orchestrator                   ← next() / record() / progress()
    │
    ▼
sub-agent (via {{pg:action.subagent_dispatcher}})      ← test / dev / review / verify / gate / fix / fix-review / fix-gate
```
...
循环:
  0. $RUNNER bootstrap <change> --detect  → 检查 response:
        • detected: true → **检测到上次 pipeline 因 terminal failure 终止**。
          用 {{pg:action.user_question}} 询问用户选择 Reset 或 Resume：
...
> 1. `{{pg:action.subagent_call}}({{pg:action.subagent_agent_parameter}}=<action.agent>, prompt="读取 <dispatch_file> 并执行任务...")`
...
1. **不要自动 reset** — 用 {{pg:action.user_question}} 询问用户：
...
2. `agent` 字段指定 sub-agent 类型（如 `pg-build/review`），**必须** 作为 {{pg:action.subagent_dispatcher}} 的 `{{pg:action.subagent_agent_parameter}}` 参数严格使用，禁止替换为其他 agent。
3. 正确用法：`{{pg:action.subagent_call}}({{pg:action.subagent_agent_parameter}}="pg-build/review", prompt="读取 {dispatch_file} 并执行任务，完成后用 pg-build-result 脚本生成返回 JSON")`，其中 `{{pg:action.subagent_agent_parameter}}` 的值从 `next()` 返回的 `agent` 字段直接读取。
