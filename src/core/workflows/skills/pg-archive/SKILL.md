...
核心移动逻辑由 `.pg/skills/src/core/workflows/skills/pg-archive/scripts/pg-archive.py` 实现，
与 `pg-build` runner 共用同一份代码，避免逻辑分叉。

```bash
python3 .pg/skills/src/core/workflows/skills/pg-archive/scripts/pg-archive.py move "${change_name}"
```
