...
def _load_agent_cli(project_root: Path) -> list[str]:
    """Resolve the selected tool's blocking agent command from its adapter."""

    state_path = project_root / ".pg" / "tool-integration.json"
    state = json.loads(state_path.read_text(encoding="utf-8"))
    tool_id = state["tool"]
    package_name = tool_id.replace("-", "_")
    descriptor_path = (
        project_root
        / ".pg"
        / "skills"
    ...
    encoded = descriptor["template_variables"]["action.agent_cli_argv_json"]
    command = json.loads(encoded)
    if not isinstance(command, list) or not all(
        isinstance(part, str) and part for part in command
    ):
        raise ValueError(f"{descriptor_path}: invalid agent CLI command")
    return command
...
"""
    ...
    # 3. Call the selected development tool's blocking LLM entry.
    agent_cli = _load_agent_cli(PROJECT_ROOT)
    print(f"  🚀 启动 {' '.join(agent_cli)} {FIX_PROD_AGENT} ...")
    try:
        fix_proc = subprocess.run(
            [*agent_cli, FIX_PROD_AGENT, str(prompt_file)],
