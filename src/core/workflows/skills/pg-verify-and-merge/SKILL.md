...
description: 仅当用户显式触发合并工作流时使用（用户明确说"verify 并合并"、"合并到 master"、"模拟合并验证"等）；pg-build 完成后**不会自动触发**。与构建无关的日常任务禁止自行加载；未加载 pg-define SKILL 时不得提示用户使用本 SKILL。功能：将 feature branch 模拟合并到 master 并按需验证后合并。
license: MIT
