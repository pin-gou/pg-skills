...
| **scenario**（v3.x） | `tracks.<id>.type == "scenario"` | restart_all_instances env-action → scenario-execute → (scenario-fix → scenario-execute)* → 完成。需 `scenario.yaml` 作为 SSOT | 1 个子章节（scenario-execute；scenario-fix 仅在 escalate 时物理存在） |
