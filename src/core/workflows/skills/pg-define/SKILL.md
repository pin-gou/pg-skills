---
name: pg-define
description: 仅当用户显式触发探索/设计/定界工作流时使用（`/1-pg-define` 命令，或用户明确说"先定界/设计/调研一下"）；与定界无关的日常任务禁止自行加载；未加载 pg-define SKILL 时不得提示用户使用 pg-* 工作流 SKILL。功能：进入探索/设计/定界模式——思考想法、调查问题、澄清需求、确定范围。包含唯一落盘环节：定界后环境验证（落盘 define-summary.yaml + describe_env）。
license: MIT
