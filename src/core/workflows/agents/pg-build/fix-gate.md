...
3. 文件中提到的 `report_seq` 与 `fix_cycle`（即 gate_cycle）是 runner 预分配的全局 seq 编号与循环序号，**必须**用 `cat > 2-build/{report_seq}-{item}-fix-gate-verify-{fix_cycle}.md << 'EOF' ... EOF` 写报告
...
| 模型 | 默认 | `{{pg:model.master}}`（需要更强语义理解）|
