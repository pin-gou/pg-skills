...
当你需要定位代码文件、理解代码结构、查找函数/类的定义时，**不要自己直接 grep/read**，而应使用 {{pg:action.subagent_dispatcher}} 调度 explore agent：

```
{{pg:action.subagent_call}}:
  description: 查找 [具体查询内容]
  prompt: |
    [查询内容描述]
    例如：查找 InstanceActions 组件的位置和导出内容
  {{pg:action.subagent_agent_parameter}}: explore
```
