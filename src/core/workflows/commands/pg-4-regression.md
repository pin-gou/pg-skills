...
- `suite` — 测试套件名（= `.pg/project.yaml` 中 `regression.suite` 段的 key），可选。留空时通过 {{pg:action.user_question}} 询问用户。
...
1. 使用 {{pg:action.skill_loader}} 加载 `pg-regression` SKILL，传入 suite 名 `{suite}`
